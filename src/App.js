  ...
  return (
    <div className="App">
      ...
      {scroll > 600 && scroll < 4000 && <img src={building} alt='building'
        ...
        style={{
          opacity: `${opacity}`,
          transform: `translate(${buildingImageX}%, ${buildingImageY}%) scale(${buildingImageScale})`
        }} />}
      {scroll < 4000 && <div className="logo-entrance-container"
        style={{
          opacity: `${opacity}`,
          left: `${leftLogoContainer}%`,
          top: `${topLogoContainer}%`,
          transform: `translate(${logoContainerX}%, ${logoContainerY}%) scale(${logoContainerScale})`
        }} >
        <img src={logoWhite}
          className='logo-entrance-img'
          style={{ height: `${logoSize}vw` }} alt='logo' />
        {scroll > 700 && <div
          className="presents-text"
          style={{ animationPlayState: `${scroll > 700 ? 'running' : 'paused'}` }}>Presents</div>}
        {scroll > 1500 && <div
          className="theme-name">
          <div className="top-theme-name">
            <div className="flip"
              style={{ animationPlayState: `${scroll > 1500 ? 'running' : 'paused'}` }}>FLIP</div>
            <div className="the"
              style={{ animationPlayState: `${scroll > 1500 ? 'running' : 'paused'}` }}>THE</div>
          </div>
          <div className="bottom-theme-name"
            onAnimationEnd={handleAnimationEnd}
            style={{
              transform: `translate(0, ${scriptPosition}px)`,
              animationName: `${animationName}`,
              animationDuration: `${animationName === 'text-appear' ? '1s' : animationName === 'flipping-animation-y' ? '0s' : '0.3s'}`,
              animationPlayState: `${scroll > 1100 ? 'running' : 'paused'}`
            }}>SCRIPT</div>
        </div>}
        {scroll > 1500 && <div className="date-and-venue">
          <div className="date"
            style={{ animationPlayState: `${scroll > 1500 ? 'running' : 'paused'}` }}>
          ...
          <div className="venue"
            style={{ animationPlayState: `${scroll > 1500 ? 'running' : 'paused'}` }}>DPU <br /> Auditorium</div>
        </div>}
      </div>}
